...
public class RangedAttackHelper {
    ...
    private static List<ItemStack> getChargedProjectiles(ItemStack itemStack) {
        ...
        if (itemStackTag != null && itemStackTag.contains("ChargedProjectiles", 9)) {
            ListNBT chargedProjectiles = itemStackTag.getList("ChargedProjectiles", 10);
            if (chargedProjectiles != null) {
                for(int i = 0; i < chargedProjectiles.size(); ++i) {
                    CompoundNBT chargedProjectilesCompound = chargedProjectiles.getCompound(i);
                    itemStacks.add(ItemStack.read(chargedProjectilesCompound));
                }
            }
        }
