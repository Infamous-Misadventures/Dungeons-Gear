            ...
            if (target.canBeAttackedWithItem() && !target.hitByEntity(player)) {
                ...
                if (attackDamage > 0.0F || enchantmentAffectsTargetBonus > 0.0F) {
                    ...
                    if (flag5) {
                        if (i > 0) {
                            if (target instanceof LivingEntity) {
                                ((LivingEntity)target).knockBack(player,(float)i * 0.5F, (double) MathHelper.sin(player.rotationYaw * 0.017453292F), (double)(-MathHelper.cos(player.rotationYaw * 0.017453292F)));
                            } else {
