...
public abstract class AbstractDungeonsCrossbowItem extends CrossbowItem implements IRangedWeapon {
    ...
    public float getProjectileVelocity(ItemStack stack) {
        ...
        if(RangedAttackHelper.hasChargedProjectile(stack, Items.FIREWORK_ROCKET)){
            if(fastProjectiles){
                return 3.2F;
            }
            else{
    ...
    void fireProjectile(World worldIn, LivingEntity shooter, Hand handIn, ItemStack crossbow, ItemStack projectile, float soundPitch, boolean isCreativeMode, float velocity, float inaccuracy, float projectileAngle) {
        if (!worldIn.isRemote) {
            ...
            IProjectile projectileentity;
            if (flag) {
                projectileentity = new FireworkRocketEntity(worldIn, projectile, shooter.getPosX(), shooter.getPosYEye() - (double)0.15F, shooter.getPosZ(), true);
            } else {
            ...
            if (shooter instanceof ICrossbowUser) {
                ...
                icrossbowuser.shoot(Objects.requireNonNull(icrossbowuser.getAttackTarget()), crossbow, projectileentity, projectileAngle);
            } else {
                Vec3d vector3d1 = shooter.getUpVector(1.0F);
                Quaternion quaternion = new Quaternion(new Vector3f(vector3d1), projectileAngle, true);
                Vec3d vector3d = shooter.getLook(1.0F);
